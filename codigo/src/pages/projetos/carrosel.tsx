...
export interface CoverflowCarouselProps {
  ...
  /** Fires when the already-centered slide is clicked. @default undefined */
  onSlideClick?: (index: number) => void;
  /** Fires when the active (centered) slide changes. @default undefined */
...
export function CoverflowCarousel({
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(event.clientX - drag.x) > 5) draggedRef.current = true;
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!draggedRef.current && pointerDownCardRef.current !== null) {
      const clicked = pointerDownCardRef.current;
      pointerDownCardRef.current = null;
      if (clicked === selected) {
        onSlideClick?.(clicked);
      } else {
        goTo(clicked);
      }
      return;
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                onPointerDown={() => {
                  pointerDownCardRef.current = index;
                }}
                role="group"
